...
// Normalize only the search key; keep original accents and spacing for display.
const normalizeSearch = value => String(value ?? '').normalize('NFD').replace(/\p{M}/gu, '').toLowerCase().replace(/\s+/gu, '');
...
const translationCacheKey='perfume-atlas-translations-v1';
let translationCache={};
try{translationCache=JSON.parse(localStorage.getItem(translationCacheKey)||'{}')}catch(error){translationCache={}}
const descriptionLanguage=p=>p.descriptionLanguage||(/[가-힣]/u.test(p.description||'')?'ko':'en');
const translationTarget=p=>descriptionLanguage(p)==='ko'?'en':'ko';
function descriptionFor(p){const target=translationTarget(p),label=target==='ko'?'KR':'EN';return `<span class="description-text">${escapeHTML(p.description)}</span> <button class="language-toggle" type="button" data-language="${target}" aria-label="Translate description to ${target==='ko'?'Korean':'English'}">${label}</button>`}
async function requestTranslation(text,target){const key=`${target}:${text}`,cached=translationCache[key];if(cached)return cached;const endpoint='https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl='+encodeURIComponent(target)+'&dt=t&q='+encodeURIComponent(text);const response=await fetch(endpoint);if(!response.ok)throw new Error('Translation request failed');const payload=await response.json(),translated=payload?.[0]?.map(part=>part?.[0]||'').join('').trim();if(!translated)throw new Error('Translation response was empty');translationCache[key]=translated;try{localStorage.setItem(translationCacheKey,JSON.stringify(translationCache))}catch(error){}return translated}
function bindDescriptionTranslation(p){const button=heroCard.querySelector('.language-toggle'),text=heroCard.querySelector('.description-text');if(!button||!text)return;const original=p.description,originalLanguage=descriptionLanguage(p),target=translationTarget(p);let translated=null,showingOriginal=true;button.addEventListener('click',async()=>{if(!showingOriginal){text.textContent=original;button.textContent=target==='ko'?'KR':'EN';button.dataset.language=target;button.setAttribute('aria-label',`Translate description to ${target==='ko'?'Korean':'English'}`);showingOriginal=true;return}button.disabled=true;button.textContent='…';button.removeAttribute('data-error');try{translated=translated||await requestTranslation(original,target);text.textContent=translated;button.textContent=originalLanguage==='ko'?'KR':'EN';button.dataset.language=originalLanguage;button.setAttribute('aria-label',`Show original ${originalLanguage==='ko'?'Korean':'English'} description`);showingOriginal=false}catch(error){button.textContent=target==='ko'?'KR':'EN';button.dataset.error='true';button.title='Translation is temporarily unavailable'}finally{button.disabled=false}})}
function noteGroups(p){return p.noteGroups||{unspecified:p.notes}}function notesMarkup(p){const groups=noteGroups(p);return [['Top',groups.top],['Middle',groups.middle],['Base',groups.base],['Key Notes',groups.unspecified]].map(([label,notes])=>notes?.length?`<div class="layer"><span class="layer-title">${label}</span>${notes.map(n=>`<button class="note-button" data-note="${escapeHTML(n)}">${escapeHTML(n)}<span>+</span></button>`).join('')}</div>`:'').join('')||'<span>Not published by the source.</span>'}
function collectionFor(p){return p.collection||'Collection not confirmed'}
function renderDetail(p){openNoteGroups.clear();connections.replaceChildren();document.querySelectorAll('.toggle').forEach(b=>{b.classList.remove('is-open');b.setAttribute('aria-expanded','false');});selected=p;backToPrevious.hidden=navigationStack.length===0;atlas.classList.add('is-focused');workspace.hidden=false;connections.hidden=true;collectionGrid.hidden=true;brandGrid.hidden=true;collectionsPanel.classList.remove('is-expanded');document.querySelector('#collectionsToggle b').textContent='+';collectionName.textContent='Collections';document.querySelector('#collectionsToggle').hidden=!p.collection||!perfumes.some(item=>item.id!==p.id&&item.collection===p.collection);document.querySelector('#brandToggle b').textContent='+';heroCard.innerHTML=`<div class="hero-head">${[p.name,p.gender,p.brand].filter(Boolean).map(escapeHTML).join(' · ')}</div><div class="hero-image"><img src="${photoFor(indexOf(p))}" alt="${escapeHTML(p.name)}"></div><div class="description">${descriptionFor(p)}<a class="source" href="${escapeHTML(p.sourceUrl)}" target="_blank" rel="noopener noreferrer">Source: ${escapeHTML(p.brand)} ↗</a></div>`;bindDescriptionTranslation(p);detailsPanel.innerHTML=`<section class="note-section"><span class="section-label">Notes</span><div class="section-body">${notesMarkup(p)}</div></section>${p.accords?.length?`<section class="note-section"><span class="section-label">Accords</span><div class="section-body">${p.accords.map(n=>`<span class="accord">${escapeHTML(n)}</span>`).join('')}</div></section>`:''}`;if(window.matchMedia('(max-width:650px)').matches)detailsPanel.append(connections);brandName.textContent=p.brand;detailsPanel.querySelectorAll('.note-button').forEach(button=>button.addEventListener('click',()=>showConnections(button.dataset.note,button)))}
